...
template <typename T>
class list_row_iter
    : public std::iterator<std::bidirectional_iterator_tag, list_row_iter<T>> {
public:
  // Type renaming
  using iterator = list_row_elem_iter<T>;
  using reverse_iterator = __detail::reverse_iter<iterator>;

  list_row_iter(list_node<T> *left, list_node<T> *right)
...
template <typename T>
class list_col_iter
    : public std::iterator<std::bidirectional_iterator_tag, list_col_iter<T>> {
public:
  // Type renaming
...
public:
  ...
  // Default constructor
  doubll2d()
      : up_left{nullptr}, up_right{nullptr}, down_left{nullptr},
        down_right{nullptr}, dim_row{0}, dim_col{0} {}
  ...
  // `col_iter` left to the given `cursor`. If the first column is deleted, then
  // return the `col_iter` right to the given `cursor`. If the deleted column is
  // the only column in the list, return `col_end()`. (implement this)
  //
...
#endif // _HW7_DOUBLL2D_HPP_#pragma once
