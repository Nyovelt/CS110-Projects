...
int check(doubll2d *list, doubll2d_elem *cursor) {
  int cursor_exist = 1; /* int cursor_exist = 1; */
...
/* check list */
  ...
  */
  int pos_y;
  pos_y = 1;
  doubll2d_elem *start_node = cursor; /* init */
...
void node_purge(doubll2d_elem *node) { /* node_purge */
  doubll2d_delete_row_helper(node);
  doubll2d_delete_col_helper(node);
}

void doubll2d_purge(doubll2d *list) { /* doubll2d_purge */
  node_purge(list->head);
  free(list);
}
